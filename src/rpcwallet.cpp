...
Value stakeforcharity(const Array &params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "stakeforcharity <EXEcoin address> <percent>\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // EXEcoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <EXEcoinpubkey>\n"
            "Return information about <EXEcoinpubkey>.");
...
// EXEcoin: reserve balance from being staked for network protection
...
// EXEcoin: check wallet integrity
...
// EXEcoin: make a public-private key pair
...
Value cccustomchange(const Array& params, bool fHelp)
{
    ...
    if(!address.IsValid()) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid EXEcoin address");
...
Value multisend(const Array &params, bool fHelp)
{
	...
	if (fHelp || params.size() != 2)
        throw runtime_error(
			...
			"multisend <EXEcoin Address> <percent>\n"
